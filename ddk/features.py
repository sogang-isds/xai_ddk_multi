...
from ddk.radarDDK import plot_radar
...
def DDK(audio, path_base="./"):
    ...
    data_audio = prepare_data(audio)
    fs = 16000
    # data_audio = my_padding(torch.tensor(data_audio), 16000*15).numpy()
    ...
    f0_filename = next(tempfile._get_candidate_names())
    # vuv_filename = next(tempfile._get_candidate_names())
    
    tmp_path = tempfile.TemporaryDirectory()
    tmp_path_str = tmp_path.name
    tmp_f0 = Path(tmp_path_str) / Path(f0_filename)
    # tmp_vuv = Path(tmp_path_str) / Path(vuv_filename)
    #print(tmp_f0,tmp_vuv)
    if not tmp_f0.is_file():
        tmp_f0.touch()
    # if not tmp_vuv.is_file():
    #     tmp_vuv.touch()
    #print(path_base+'./praat '+path_base+'F0_Praat.praat '+audio+' '+str(tmp_f0) + ' 75 500 0.02')
    ...
    intensity = call(sound, "To Intensity", f0min, 0, True)
    ...
    # VUV(audio, str(tmp_vuv), path_base, time_stepF0, low_f0, high_f0, maxPeriodVUV, averagePeriodvuv)
    # segmentsV, fs=decode_Texgrid(str(tmp_vuv) , audio, 'Voiced')
    # segmentsU, fs=decode_Texgrid(str(tmp_vuv), audio, 'Unvoiced')
    ...
    #print()
    return [F0varsemi, F0std, meanIntensity, stdevIntensity, maxIntensity]
...
def VUV(audio, results, path_base, time_stepF0, lowf0, highf0, maxVUVPeriod, averageVUVPeriod):
    ...
    os.system(f"{os.path.join(path_base, 'praat')} {os.path.join(path_base, 'vuv.praat')} {audio} {results} {str(lowf0)} {str(highf0)} {str(time_stepF0)} {str(maxVUVPeriod)} {str(averageVUVPeriod)}")
